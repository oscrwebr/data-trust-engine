...
async def test_ingest(application: Annotated[ConfidentialClientApplication, Depends(application)], user: Annotated[User, Depends(get_user_from_access_token)], db: Annotated[Session, Depends(get_database)]):
    print(user)
    access_token = service.get_user_access(application=application, user_id=user.user_id, db=db)
    if access_token:
        output = service.get_all_files(access_token=access_token, id=user.user_id, db=db)
        return output
