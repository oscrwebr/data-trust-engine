...
def insert_unknown_user_folders(user_folders: list, db: Session):
    status = {
        "status": 200
    }
    try:
        db.execute(insert(UnknownUserFolders), user_folders)
        db.commit()
    except Exception as e:
        status["description"] = f"An error ocurred: {type(e).__name__} - {e}"
        status["status"] = 403
...
def insert_unknown_user_files(user_files: list, db: Session):
    status = {
        "status": 200
    }
    try:
        db.execute(insert(UnknownUserFiles), user_files)
        db.commit()
    except Exception as e:
        status["description"] = f"An error ocurred: {type(e).__name__} - {e}"
        status["status"] = 403
